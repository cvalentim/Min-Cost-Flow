...
class Graph_Mean
{
		...
		vector<int> find_cycle(int start, const vector<int>& prev){
				...
				if (u == -1){
					...
					for (int i = 0; i < n; ++i) cerr<<"prev["<<i<<"]"<<" = "<<prev[i]<<" ";
					cerr<<endl;
					//print_graph();
					assert(0 == 1);
				}
		...
		bool improve_flow(){
				...
				// This way we can run just one bellman at time
				int source = n;
				for (int i = 0; i < n; ++i)
						set_edge(source, i, 0, 0);

				double eps = 1e-10;
				double start = -10000001, end = +10000001;
				vector<int> cycle;

				// binary search to find the minimum mean cycle 
				while (fabs(end - start) > eps){
						double mean = (end + start)/2;
						bool found = false;
						...
						for (int step = 0; step < n + 1 - 1;  ++step)
								for (int i = 0; i < n + 1; ++i)
										for (int j = 0; j < n + 1; ++j) if (i != j && get_edge_cap(i, j) > 0)
												if (dist[j] > dist[i] + ((double)cost[i][j] - mean) + eps){
														dist[j] = dist[i] + ((double)cost[i][j] - mean);
														prev[j] = i;
												}
						...
						for (int i = 0; i < n + 1; ++i)
								for (int j = 0; j < n + 1; ++j) if (i != j && get_edge_cap(i,j) > 0)
										if (dist[j] > dist[i] + ((double)cost[i][j] - mean) + eps){ // negative cycle found
												cycle = find_cycle(j, prev);
												found = true;
												goto out;
										}
out:
				...
				// not found a negative cycle
				if (!cycle.size()) return false;
				
				// found a cycle, update residual network
		...
		Graph_Mean(int n): INF(0x3f3f3f3f){
				this->n = n;
