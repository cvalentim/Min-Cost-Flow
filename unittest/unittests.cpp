...
*/
	...
	*/
	void test_minimum_path(int nv, int ne)
	{
		...
		vector<vector<int> > mp_graph(n), mp_cost(n);
		for (int i = 0; i < n; ++i){
			mp_graph[i].resize(n, 0);
			mp_cost[i].resize(n,0);
		}
		...
		for (int i = 0; i < m - 1; ++i)
		{
			...
			do
			{
				u = rand()%n;
				v = rand()%n;
				c = (rand() + 1) %1000; // no zero capacity
			}while (grafo_mean.is_edge_set(u, v) || grafo_mean.is_edge_set(v, u));

			grafo_mean.set_edge(u, v, c, 1);
			grafo_cycle.set_edge(u, v, c, 1);
